...
/**
 * Select which academic year to configure under Academic structure (terms, sequences).
 * Not for operational modules — those use the current year from AcademicYearProvider.
 */
export function AcademicYearStructureSelect({
  value,
  onValueChange,
  className,
}: {
  ...
  return (
    <Select value={value} onValueChange={onValueChange} disabled={isLoading || years.length === 0}>
      <SelectTrigger className={className ?? 'w-full max-w-xs'}>
        <SelectValue placeholder={isLoading ? 'Loading years…' : 'Select year to configure'} />
      </SelectTrigger>
      <SelectContent>
        {years.map((y) => (
