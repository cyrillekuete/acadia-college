...
import { formatRoleWithPermissions } from '@/lib/acadia/format-role-with-permissions';
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(formatRoleWithPermissions(role));
  } catch {
    return NextResponse.json(
  ...
  try {
    ...
    const updatedRole = await prisma.$transaction(
      async (tx: Prisma.TransactionClient) => {
        ...
        const roleWithPermissions = await tx.userRole.findUnique({
          where: { id },
          include: {
            permissions: {
    ...
    return NextResponse.json(formatRoleWithPermissions(updatedRole));
  } catch {
    return NextResponse.json(
      { message: 'Oops! Something went wrong. Please try again in a moment.' },
      { status: 500 },
